import { normalizeBaseUrl, assertPairingCode } from './protocol.js';
...
async function restore() {
  ...
  if (saved.pendingPrompt) {
    show(`已有待填入提示：${saved.pendingPrompt.reviewId}\n${saved.pendingPrompt.prompt.length} 个字符`);
  } else if (saved.sessionExpiresAt) {
    show(`已配对，会话到期：${new Date(saved.sessionExpiresAt).toLocaleTimeString()}`);
  }
}

$('pair').addEventListener('click', async () => {
  const targetSite = $('targetSite').value;
  const label = SITE_LABELS[targetSite];
  try {
    if (!label) throw new Error('目标站点不受支持。');
    const baseUrl = normalizeBaseUrl($('baseUrl').value.trim());
    const pairingCode = assertPairingCode($('pairingCode').value.trim());
    const permissionRequest = chrome.permissions.request({
      origins: [`https://${targetSite}/*`],
    });
...
$('fill').addEventListener('click', async () => {
  try {
    ...
    if (hostname !== prompt.targetSite) {
      throw new Error(`提示目标为 ${prompt.targetSite}，当前页面不匹配。`);
    }
