...
export interface ReviewPackSecuritySummary {
  readonly scanId: string; readonly contentFingerprint: string;
  readonly hardBlockCount: number; readonly blockCount: number; readonly warnCount: number;
...
export interface ReviewPackBuildInput {
  ...
  readonly instructions: string; readonly requirements?: string; readonly implementationReport?: string;
  readonly diff?: string; readonly files: readonly ReviewPackFile[];
  readonly excluded?: readonly { path: string; reason: string }[];
  readonly maxSizeKb?: number; readonly generatedAt?: Date; readonly reviewId?: string;
}
export interface ReviewPackManifest {
  readonly schemaVersion: 1; readonly workspaceId: string; readonly reviewId: string;
  readonly repositoryDisplayName: string; readonly generatedAt: string; readonly reviewMode: ReviewMode;
  readonly git: { readonly base: string; readonly target: string };
  readonly security: { readonly scanId: string; readonly hardBlocked: number; readonly blocked: number; readonly warnings: number; readonly info: number; readonly confirmedWarnings: number };
...
export interface ReviewPackBuildResult {
  readonly markdown: string; readonly manifest: ReviewPackManifest; readonly workspaceId: string;
  readonly reviewId: string; readonly directoryName: string; readonly byteLength: number; readonly zip: Uint8Array;
}
...
export function normalizeRepositoryIdentity(identity: string): string {
  ...
  if (/^[A-Za-z]:[\\/]/.test(value)) return value.replace(/\\/g, '/').replace(/\/+$/, '');
  const scp = /^([^@\s]+)@([^:\s]+):(.+)$/.exec(value);
  if (scp) return `ssh://${scp[2].toLowerCase()}/${scp[3].replace(/\.git\/?$/i, '').replace(/\/+$/, '')}`;
  try {
...
export function createWorkspaceId(repositoryIdentity: string): string { return sha256(normalizeRepositoryIdentity(repositoryIdentity)).slice(0, 16); }
...
export async function createUniqueReviewId(
  exists: (reviewId: string) => boolean | Promise<boolean>, date = new Date(),
  random: (size: number) => Uint8Array = randomBytes, attempts = 20,
): Promise<string> {
...
function safeText(value: string, maxLength: number): string { return value.replace(/[\0\r\n]+/g, ' ').trim().slice(0, maxLength); }
...
function section(title: string, body: string | undefined): string { return body?.trim() ? `\n## ${title}\n\n${body.trim()}\n` : ''; }
...
function frontMatter(manifest: ReviewPackManifest): string {
  return ['---', `schemaVersion: ${manifest.schemaVersion}`, `workspaceId: ${manifest.workspaceId}`,
    `reviewId: ${manifest.reviewId}`, `repository: ${JSON.stringify(manifest.repositoryDisplayName)}`,
    `generatedAt: ${JSON.stringify(manifest.generatedAt)}`, `reviewMode: ${manifest.reviewMode}`,
    `gitBase: ${JSON.stringify(manifest.git.base)}`, `gitTarget: ${JSON.stringify(manifest.git.target)}`, '---', ''].join('\n');
}
...
export class ReviewPackBuilder {
  async build(input: ReviewPackBuildInput): Promise<ReviewPackBuildResult> {
    ...
    const baseManifest: ReviewPackManifest = {
      schemaVersion: 1, workspaceId, reviewId, repositoryDisplayName, generatedAt: generatedAt.toISOString(), reviewMode: input.reviewMode,
      git: { base: safeText(input.gitBase, 200), target: safeText(input.gitTarget, 200) },
      security: { scanId: input.security.scanId, hardBlocked: input.security.hardBlockCount, blocked: input.security.blockCount,
        warnings: input.security.warnCount, info: input.security.infoCount, confirmedWarnings: input.security.confirmedWarnCount },
    ...
    const appendBudgeted = (label: string, body: string): boolean => {
      ...
      const limited = truncateUtf8(body, remaining); markdown += limited.value;
    ...
    const zip = createZip([
      ...
      { name: `${directoryName}/manifest.json`, data: Buffer.from(JSON.stringify(manifest, null, 2) + '\n', 'utf8') },
    ]);
...
function createZip(entries: readonly ZipEntry[]): Uint8Array {
  ...
  for (const entry of entries) {
    const name = Buffer.from(entry.name, 'utf8'); const crc = crc32(entry.data); const header = Buffer.alloc(30);
    ...
    header.writeUInt32LE(entry.data.length, 22); header.writeUInt16LE(name.length, 26); local.push(header, name, entry.data);
  ...
  const centralSize = central.reduce((total, item) => total + item.length, 0); const end = Buffer.alloc(22);
