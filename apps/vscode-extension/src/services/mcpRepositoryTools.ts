...
export class McpRepositoryTools {
  ...
  async call(name: string, rawArguments: unknown, signal?: AbortSignal): Promise<McpToolCallResult> {
    try {
      ...
      switch (name) {
        case 'repository_summary': return this.#repositorySummary(signal);
        case 'git_status': return this.#gitStatus(signal);
        case 'recent_commits': return this.#recentCommits(args, signal);
        case 'get_diff': return this.#getDiff(args, signal);
        case 'list_files': return this.#listFiles(args, signal);
        case 'read_file': return this.#readFile(args);
        case 'search_code': return this.#searchCode(args, signal);
  ...
  async #repositorySummary(signal?: AbortSignal): Promise<McpToolCallResult> {
    const [branch, head, status, latestCommitOutput] = await Promise.all([
      ...
      this.#git(['log', '-1', '--date=iso-strict', '--pretty=format:%H%x09%an%x09%ad%x09%s'], signal),
    ]);
    ...
    return toolSuccess({
      ...
      latestCommit: parseCommitLine(latestCommitOutput),
      access: 'read-only',
    });
  }

  async #gitStatus(signal?: AbortSignal): Promise<McpToolCallResult> {
    const status = await this.#git(['status', '--short', '--branch', '--untracked-files=all'], signal);
    return toolSuccess({ repository: this.#displayName, status: truncateText(status, this.#maxResultBytes) });
  }

  async #recentCommits(args: Record<string, unknown>, signal?: AbortSignal): Promise<McpToolCallResult> {
    const count = readInteger(args.count, 5, 1, 30);
    const output = await this.#git([
      'log', '-n', String(count), '--date=iso-strict', '--pretty=format:%H%x09%an%x09%ad%x09%s',
    ], signal);
    return toolSuccess({ repository: this.#displayName, commits: nonEmptyLines(output).map(parseCommitLine) });
  }

  async #getDiff(args: Record<string, unknown>, signal?: AbortSignal): Promise<McpToolCallResult> {
    ...
    if (scope === 'working') {
      ...
      const ranged = await this.#collectDiff(
        [await this.#verifyCommit(baseRef, signal), await this.#verifyCommit(headRef, signal)],
        pathFilter,
        signal,
      );
  ...
  async #collectDiff(
    ...
    const namesOutput = await this.#git([
      'diff', ...safeFlags, '--name-only', '-z', ...refArguments, '--', ...pathArguments,
    ], signal);
    const changedFiles = namesOutput.split('\0').filter(Boolean);
    const selectedFiles = changedFiles.slice(0, MAX_DIFF_FILES);
    const pieces: string[] = [];
    let byteCount = 0;
    let truncated = changedFiles.length > selectedFiles.length;

    for (const file of selectedFiles) {
      const normalizedFile = normalizeRepositoryRelativePath(file, 'Git diff path');
      const diff = await this.#git(['diff', ...safeFlags, ...refArguments, '--', normalizedFile], signal);
      if (!diff) continue;
    ...
    return { output: pieces.join('\n'), truncated, excludedSensitiveFiles: 0, includedFiles: pieces.length };
  }

  async #listFiles(args: Record<string, unknown>, signal?: AbortSignal): Promise<McpToolCallResult> {
    ...
    const eligible = output.split('\0').filter(Boolean)
      .filter((file) => !prefix || normalizeRepoPath(file).startsWith(prefix));
    const files = eligible.slice(0, limit);
    return toolSuccess({ repository: this.#displayName, files, truncated: eligible.length > files.length });
  }
  ...
  async #readFile(args: Record<string, unknown>): Promise<McpToolCallResult> {
    ...
    if (fileStat.size > MAX_READ_BYTES) throw new Error(`File exceeds the ${MAX_READ_BYTES}-byte read limit.`);
  ...
  async #searchCode(args: Record<string, unknown>, signal?: AbortSignal): Promise<McpToolCallResult> {
    const query = readString(args.query, 'query');
    if (query.length > 200) throw new Error('Search query is too long.');
    const prefix = normalizeOptionalPrefix(readOptionalString(args.prefix));
    const maxResults = readInteger(args.maxResults, DEFAULT_SEARCH_RESULTS, 1, MAX_SEARCH_RESULTS);
    const output = await this.#git(['ls-files', '-co', '--exclude-standard', '-z'], signal);
    ...
    for (const relativePath of candidates) {
      ...
      try {
        ...
        for (let index = 0; index < lines.length && matches.length < maxResults; index += 1) {
          ...
          matches.push({ path: normalizeRepoPath(relativePath), line: index + 1, snippet: line.trim().slice(0, 500) });
        }
      } catch {
        // Files may disappear while the workspace changes. Skip them without widening access.
      }
    }
    return toolSuccess({ repository: this.#displayName, query, matches, truncated: matches.length === maxResults });
  }

  async #verifyCommit(ref: string, signal?: AbortSignal): Promise<string> {
    const output = await this.#git(['rev-parse', '--verify', '--quiet', '--end-of-options', `${ref}^{commit}`], signal);
...
const TOOL_DEFINITIONS: readonly McpToolDefinition[] = [
  {
    name: 'repository_summary', title: 'Repository summary',
    description: 'Read the bound repository identity, current branch, HEAD commit, latest commit, and working-tree summary.',
    inputSchema: { type: 'object', additionalProperties: false }, annotations: READ_ONLY_ANNOTATIONS,
  },
  {
  ...
  {
    name: 'recent_commits', title: 'Recent commits', description: 'List recent commits so you can choose a sensible review range.',
    inputSchema: { type: 'object', additionalProperties: false, properties: { count: { type: 'integer', minimum: 1, maximum: 30, default: 5 } } },
    annotations: READ_ONLY_ANNOTATIONS,
  },
  {
    name: 'get_diff', title: 'Read Git diff',
    description: 'Read working-tree, staged, or commit-range changes for repository files.',
    inputSchema: {
      type: 'object', additionalProperties: false,
      properties: {
        scope: { type: 'string', enum: ['working', 'staged', 'range'], default: 'working' },
        baseRef: { type: 'string' }, headRef: { type: 'string', default: 'HEAD' },
        path: { type: 'string', description: 'Optional repository-relative path filter.' },
      },
      allOf: [{ if: { properties: { scope: { const: 'range' } }, required: ['scope'] }, then: { required: ['baseRef'] } }],
    },
    annotations: READ_ONLY_ANNOTATIONS,
  },
  {
    name: 'list_files', title: 'List repository files',
    description: 'List tracked and untracked, non-ignored repository files.',
  ...
  {
    name: 'read_file', title: 'Read repository file',
    description: 'Read a bounded line range from any text file inside the bound repository. Absolute paths, .git, symlink escapes, binary files, and oversized files are rejected.',
    inputSchema: { type: 'object', additionalProperties: false, properties: {
      path: { type: 'string' }, startLine: { type: 'integer', minimum: 1, default: 1 }, endLine: { type: 'integer', minimum: 1 },
    }, required: ['path'] }, annotations: READ_ONLY_ANNOTATIONS,
  },
  {
    name: 'search_code', title: 'Search repository code',
    description: 'Search non-ignored text files for a literal, case-insensitive string and return bounded matching lines.',
    inputSchema: { type: 'object', additionalProperties: false, properties: {
      query: { type: 'string', minLength: 1, maxLength: 200 }, prefix: { type: 'string' },
      maxResults: { type: 'integer', minimum: 1, maximum: 100, default: 40 },
    }, required: ['query'] }, annotations: READ_ONLY_ANNOTATIONS,
  },
];
...
function toolError(message: string): McpToolCallResult { return { content: [{ type: 'text', text: message }], isError: true }; }
...
function readEnum<const T extends readonly string[]>(value: unknown, allowed: T, fallback: T[number]): T[number] {
  if (value === undefined || value === null) return fallback;
  if (typeof value !== 'string' || !allowed.includes(value as T[number])) throw new Error(`Expected one of: ${allowed.join(', ')}.`);
...
function isAbsoluteLike(value: string): boolean {
  return path.isAbsolute(value) || path.posix.isAbsolute(normalizeRepoPath(value)) || /^[A-Za-z]:[\\/]/.test(value) || /^\\\\/.test(value);
}
function normalizeRepositoryRelativePath(value: string, label: string): string {
  ...
  if (!normalized || normalized === '.' || normalized === '..' || normalized.startsWith('../')) throw new Error(`${label} resolves outside the repository.`);
  if (normalized === '.git' || normalized.startsWith('.git/')) throw new Error('The .git directory is not readable through MCP.');
  return normalized;
}
function normalizeOptionalFilePath(value: string | undefined): string | undefined {
  return value ? normalizeRepositoryRelativePath(value, 'Git diff path') : undefined;
}
function normalizeOptionalPrefix(value: string | undefined): string | undefined {
  if (!value) return undefined;
  return `${normalizeRepositoryRelativePath(value, 'Prefix').replace(/\/+$/, '')}/`;
}
function looksBinary(value: Buffer): boolean { return value.subarray(0, Math.min(value.length, 8_192)).includes(0); }
function sanitizeRemoteUrl(value: string): string {
  const trimmed = value.trim();
  try {
    const parsed = new URL(trimmed); parsed.username = ''; parsed.password = '';
    return parsed.toString().replace('//@', '//');
  } catch {
    const scpStyle = trimmed.match(/^(?:[^@/\s]+@)?([^:/\s]+):(.+)$/);
    if (scpStyle && !/^[A-Za-z]:[\\/]/.test(trimmed)) return `ssh://${scpStyle[1]}/${scpStyle[2]}`;
    return trimmed.replace(/([a-z][a-z0-9+.-]*:\/\/)[^\s/@]+(?::[^\s/@]*)?@/gi, '$1[REDACTED]@');
  }
}
