...
const READ_FOR_EDIT_DEFINITION: RuntimeToolDefinition = {
...
const WRITE_FILES_DEFINITION: RuntimeToolDefinition = {
  name: 'write_files',
  title: 'Write confirmed repository files',
  description:
    'Create or completely replace up to 20 bounded text files inside the bound repository. Existing files require the exact SHA-256 returned by read_file_for_edit. Every effective batch is shown in VS Code and requires explicit user confirmation. This tool cannot delete files, access .git, escape the repository, execute commands, or perform Git operations.',
  inputSchema: {
    type: 'object',
    additionalProperties: false,
    properties: {
...
export class McpWritableRepositoryTools extends McpRepositoryTools {
  ...
  async #prepareChanges(requested: readonly RequestedChange[]): Promise<PreparedChange[]> {
    ...
    for (const request of requested) {
      ...
      if (paths.has(target.path)) throw new Error(`Duplicate write path: ${target.path}`);
...
function readChanges(value: unknown): readonly RequestedChange[] {
  ...
  return value.map((entry, index) => {
    ...
    if (typeof item.content !== 'string') {
      throw new Error(`changes[${index}].content must be a string.`);
    }
    return {
