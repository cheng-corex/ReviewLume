...
export async function saveAutomaticReviewPack(
  ...
  if (format === 'markdown' || format === 'both') {
    const markdownDirectory = path.join(exportRoot, pack.reviewId);
    await fs.mkdir(markdownDirectory, { recursive: false }).catch((error: unknown) => {
      const code = typeof error === 'object' && error !== null && 'code' in error
        ? String((error as { code: unknown }).code)
        : '';
      if (code !== 'EEXIST') throw error;
    });
    const markdownPath = path.join(markdownDirectory, 'REVIEW_REQUEST.md');
