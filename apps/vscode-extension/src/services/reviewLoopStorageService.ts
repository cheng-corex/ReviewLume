...
export class ReviewLoopStorageService {
  ...
  async saveReReviewPrompt(
    ...
    const roundIssueIds = issueIds ?? state.implementationSummary?.issueIds;
    if (!roundIssueIds || roundIssueIds.length === 0 || new Set(roundIssueIds).size !== roundIssueIds.length) {
