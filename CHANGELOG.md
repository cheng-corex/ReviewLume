...
- Folder Project Support for the read-only ChatGPT MCP connector:
  ...
  - ReviewLume resolves a `ProjectContext` automatically as either a Git Project or Folder Project while keeping one active MCP connection bound to one outer project root.
  - Folder Projects expose `project_summary`, `list_files`, `read_file`, and `search_code` across the authorized Folder root.
  - Folder Projects also expose `list_git_repositories` plus explicitly repository-scoped `repository_summary`, `git_status`, `recent_commits`, and `get_diff` for real nested Git repositories inside that root.
  - Nested Git queries require one child repository path returned by `list_git_repositories`; the Folder root has no aggregate Git history and separate child repositories are never combined into synthetic Git state.
  - Nested repository discovery is bounded, does not follow directory symlink/junction escapes, and requires both Git top-level and absolute Git metadata to resolve inside the authorized Folder root.
  - Folder Projects do not discover, run, approve, or expose Local Verification, including for nested Git repositories; the existing repository-bound verification safety model remains direct Git Project only.
  - The primary action is now `Connect Current Project to ChatGPT`, and connected status identifies the detected outer `Git` or `Folder` project kind.
  ...
  - Folder file enumeration is process-free and bounded, canonicalizes the root, does not follow symlink/junction-like entries, verifies real paths remain inside the root, skips VCS/dependency/build/credential-store trees, and rejects obvious credential-like direct-file paths.
...
- Folder direct-file tools add a stricter path denylist for obvious credential-like files without claiming content-level DLP; environment templates such as `.env.example` remain readable.
- Folder nested Git discovery validates candidate top-level and Git metadata containment before exposing the existing read-only Git query contract; external Git metadata and repository selectors outside the Folder root are rejected.
- Explicit nested Git status/history/diff uses the existing Git read semantics and is not sanitized by the Folder direct-file filename denylist.
- ReviewLume still exposes no MCP shell, terminal, arbitrary command runner, general process-start, file write, delete, patch, or Git-mutation tool.
...
- Folder roots have no aggregate Git history semantics; nested Git queries inspect one discovered child repository at a time.
- Folder Projects have no Local Verification, including for child repositories reached through Folder mode.
