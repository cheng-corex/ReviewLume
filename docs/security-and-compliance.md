...
- 一次 MCP connection 只绑定一个 canonical Project Root；
...
- 绝对路径、父目录逃逸、`.git`、root 外 realpath、目录、二进制和超限内容不能通过文件读取工具返回；
- 停止连接或 Extension Host 退出后，本地端口和 Token 失效；
- Git Project 与 Folder Project 的真实隐私边界必须分别说明，不宣传不存在的 DLP 能力。
...
└────────────────┴────────────────┘
        ↓
one MCP connection = one Project Root
```
...
3. 成功 → Git Project；
4. 无 repository 或 Git discovery 不可用 → Folder Project；
5. Folder 模式使用更小 capability set，不伪造 Git 信息。

多 Workspace Folder 时仍必须选择一个 root。本轮不实现 Multi Project Registry，也不跨 root 混读。
...
ChatGPT 只能看到当前 Project Kind 注册的工具
  ↓
ReviewLume 经过 project root、路径、类型、大小和速率门禁返回结果
  ↓
...
即使客户端尝试隐藏调用，也必须返回 unsupported，而不是模拟 Git/Verification 结果。

Folder Project 的 `project_summary` 必须明确：

- `projectKind = folder`；
- 无可靠 Git history；
- 无 Local Verification；
- 不得推断 branch、HEAD、commit、staged/unstaged、diff 或“最近改了什么”。
...
## 6. Folder Project 枚举与敏感路径

没有 Git index 时，Folder Project 使用受限 filesystem enumeration，而不是执行 `git ls-files`：
...
Folder Project 还额外拒绝明显 credential-like path，例如：
...
Git Project 继续使用受控只读 Git allowlist：
...
Local Verification 仍然只属于 **Git Project**。
...
不得为了 Folder Project 拆掉现有 repository identity / approval / HEAD / working-tree / runner binding 安全模型。

## 9. 威胁模型

### 9.1 Prompt injection / 恶意项目内容

项目中的 README、源码注释、测试、配置或文件名可能要求模型执行命令、读取 root 外文件或扩大权限。

应对：工具集由扩展代码固定；Project 内容不能动态注册工具；路径门禁独立于内容；无写/执行 MCP capability。

### 9.2 Path traversal 与链接逃逸

应对：relative-path validation + canonical root + `realpath` containment。Folder enumeration 不跟随 link entry，显式 read 即使访问 link，也会在 shared reader 的 realpath boundary 被拒绝。

### 9.3 Sensitive data exposure

Git 与 Folder 的 filename policy 不同，但两者都不是完整内容 DLP。用户必须在连接前移除、轮换或脱敏真实秘密，并只连接有权提供给 OpenAI 的数据。

### 9.4 Local endpoint abuse

应对：loopback-only、fresh token、Bearer/专用 Tunnel header、Origin/content-type/body/rate limits、停止后失效。

### 9.5 Tunnel credential contamination
